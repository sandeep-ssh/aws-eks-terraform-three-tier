...
# Safely select subnets for EKS cluster and node group
locals {
  eks_subnet_ids = length(data.aws_subnets.all_supported.ids) >= 2 ? slice(data.aws_subnets.all_supported.ids, 0, 2) : data.aws_subnets.all_supported.ids
  eks_public_subnets = length(data.aws_subnets.all_supported.ids) >= 3 ? slice(data.aws_subnets.all_supported.ids, 0, 3) : data.aws_subnets.all_supported.ids
}
...
############################################
# Launch Template (Public IP enabled)
############################################
...
############################################
# EKS Node Group (Multi-AZ Public)
############################################
resource "aws_eks_node_group" "example" {
  cluster_name    = aws_eks_cluster.eks_cluster.name
  node_group_name = "eks-node-group"
  node_role_arn   = aws_iam_role.nodegroup_role.arn
  subnet_ids      = local.eks_public_subnets
